...
class Orr(Node):
    ...
    def mapToIR(self, ctx: EmitterContext):
        return _primMap(ctx, self, low_prim.Orr(), [self.item], [], Orr._tf)
...
class Andr(Node):
    ...
    def mapToIR(self, ctx: EmitterContext):
        return _primMap(ctx, self, low_prim.Andr(), [self.item], [], Andr._tf)
...
class Xorr(Node):
    ...
    def mapToIR(self, ctx: EmitterContext):
        return _primMap(ctx, self, low_prim.Xorr(), [self.item], [], Xorr._tf)
...
class And(Node):
    ...
    def __post_init__(self):
        ...
        self.width = max(self.lhs.width, self.rhs.width)
...
class Mul(Node):
    ...
    def __post_init__(self):
        super().__post_init__()
        self.typ = self.lhs.typ
        self.width = self.lhs.width + self.rhs.width
    ...
    def __post_init__(self):
        ...
        if isinstance(self.ref.typ, Vec):
            ...
            if isinstance(self.index, slice):
                ...
                self.typ = self.ref.typ.data_typ
        else:
